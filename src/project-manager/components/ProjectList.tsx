...
export const ProjectList = ({ projects, onSelectProject, onProjectsChange, onClose }: ProjectListProps) => {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-900">
      ...
      {showCreateDialog && (
        <div
          className="fixed inset-0 bg-black/60 flex items-end justify-center z-[10000] animate-fade-in"
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowCreateDialog(false);
          }}
        >
          <div className="bg-slate-900 p-5 rounded-t-card shadow-overlay w-full max-w-md animate-slide-up" style={{ paddingBottom: 'calc(20px + var(--safe-area-inset-bottom))' }}>
            <div className="flex items-center justify-center mb-4">
              <div className="w-10 h-1 bg-slate-600 rounded-full" />
            </div>
            <h3 className="text-white font-bold text-base mb-4">新建项目</h3>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-lg text-white placeholder-slate-500 text-sm focus:outline-none focus:border-cyan-500 transition-colors mb-4"
              autoFocus
            />
            ...
            <div className="flex gap-2 mb-5">
              <button
                ...
                className={`flex-1 min-h-[44px] py-2.5 rounded-lg text-sm font-medium transition-colors ${
                  projectType === 'folder'
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 min-h-[44px] py-2.5 bg-slate-700 text-white rounded-lg text-sm font-medium active:bg-slate-600 transition-colors"
              >
                取消
              </button>
              <button
                ...
                className="flex-1 min-h-[44px] py-2.5 bg-cyan-500 text-white rounded-lg text-sm font-medium active:bg-cyan-600 disabled:bg-slate-800 disabled:text-slate-500 transition-colors"
              >
                {creating ? '创建中...' : '创建'}
              </button>
            </div>
